        ...
        .then((parsed) => {
            ...
            if (parsed.feed) {
                for (let entry of parsed.feed.entry) {
                    ...
                    toEmit.push({ title: entry.title[0], link: entry.link[0].$.href, updated_time: updated });
                }
            } else {
                for (let entry of parsed.rss.channel[0].item) {
                    let updated = new Date(entry.pubDate[0]);
                    toEmit.push({ title: entry.title[0], link: entry.link[0], updated_time: updated, description: entry.description[0] });
                }
            }
